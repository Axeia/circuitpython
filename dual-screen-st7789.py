...
GND  --> Board GND
VCC  --> Board 3.3v
SCL  --> Board IO12 [Board SCK] (Fixed - do not change. Both displays share this pin)
SDA  --> Board IO11 [Board MOSI] (Fixed - do not change. Both displays share this pin)
...
Display2: (pins are opposite to Display 1s pins)
RES  --> Board IO2 (Not fixed, just pick an IO pin)
DC   --> Board IO42 (Not fixed, just pick an IO pin)
CS   --> Board IO41 (Not fixed, just pick an IO pin)
'''
...
# Set up the display
RESET_PIN1 = board.IO5 # Can be changed
DC_PIN1    = board.IO6 # Can be changed
CS_PIN1    = board.IO7 # Can be changed

# Create the ST7789 displays for the first and second screens
display1 = ST7789(
    spi,
    rotation=90,
    width=172,
    ...
    y_offset=0,
    baudrate=BAUDRATE,
    cs=digitalio.DigitalInOut(CS_PIN1),  # Use the appropriate CS_PIN for the first display
    dc=digitalio.DigitalInOut(DC_PIN1),  # Use the appropriate DC_PIN for the first display
    rst=digitalio.DigitalInOut(RESET_PIN1),  # Use the appropriate RESET_PIN for the first display
)


# Set up the display
RESET_PIN2 = board.IO2  # Can be changed
DC_PIN2    = board.IO42 # Can be changed
CS_PIN2    = board.IO41 # Can be changed
...
display2 = ST7789(
    ...
    y_offset=0,
    baudrate=BAUDRATE,
    cs=digitalio.DigitalInOut(CS_PIN2),  # Use the appropriate CS_PIN for the second display
    dc=digitalio.DigitalInOut(DC_PIN2),  # Use the appropriate DC_PIN for the second display
    rst=digitalio.DigitalInOut(RESET_PIN2),  # Use the appropriate RESET_PIN for the second display
)
